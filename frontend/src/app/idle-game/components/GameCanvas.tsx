...
import { GameEngine, GW, GH, UpgradeDef, GameSave, GamePhase, DURATION } from '../game/engine';
...
export default function GameCanvas({ save, onExit }: Props) {
  ...
  const handlePointer = useCallback((e: React.PointerEvent) => {
    ...
    const wx = (e.clientX - rect.left) * (GW / rect.width);
    const wy = (e.clientY - rect.top) * (GH / rect.height);
  ...
  return (
    <div className="fixed inset-0 bg-black flex items-center justify-center z-[60]" style={{ touchAction: 'none' }} data-testid="game-canvas-container">
      <div className="relative w-full h-full max-w-[100vh] max-h-[100vw] aspect-square m-auto">
        <canvas
          ref={canvasRef}
          width={GW}
          height={GH}
        ...
        />
        {/* HUD */}
        <div className="absolute inset-0 pointer-events-none" style={{ fontFamily: "'Press Start 2P', monospace", fontSize: '8px' }}>
          {/* Top bar */}
          <div className="flex justify-between items-start p-2">
            <div className="flex flex-col gap-1">
              <div className="bg-black/70 px-2 py-1 rounded text-white text-[10px]" data-testid="hud-level">Lv.{hud.level}</div>
              <div className="bg-black/70 px-2 py-1 rounded text-white text-[10px]" data-testid="hud-kills">Kills: {hud.kills}</div>
            </div>
            <div className="bg-black/70 px-3 py-1.5 rounded text-white text-xs font-bold tracking-wider" data-testid="hud-timer">
              {fmtTime(hud.time)}
            </div>
            <div className="flex flex-col gap-1 items-end">
              <div className="bg-black/70 px-2 py-1 rounded text-yellow-400 text-[10px]" data-testid="hud-gold">
                <span className="text-yellow-300">$</span> {hud.gold}
              </div>
              <button onClick={toggleSpeed} className="bg-black/70 px-2 py-1 rounded text-lime-400 text-[10px] pointer-events-auto active:bg-black/90" data-testid="speed-toggle">
                x{speed}
              </button>
            </div>
          </div>
          {/* HP Bar */}
          <div className="absolute left-1/2 -translate-x-1/2 w-40" style={{ bottom: '14%' }}>
            <div className="bg-black/70 rounded p-1">
              <div className="h-2.5 bg-red-900/80 rounded-sm overflow-hidden">
              ...
              <div className="text-center text-[8px] text-white mt-0.5">{hud.hp}/{hud.maxHp}</div>
            </div>
          </div>
          {/* XP Bar */}
        ...
        {/* Level Up Overlay */}
        {phase === 'levelup' && (
          <div className="absolute inset-0 bg-black/75 flex items-center justify-center p-3" data-testid="levelup-overlay">
        ...
        {(phase === 'victory' || phase === 'defeat') && (
          <div className="absolute inset-0 bg-black/85 flex items-center justify-center p-4" data-testid="game-end-overlay">
